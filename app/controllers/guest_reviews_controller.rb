class GuestReviewsController < ApplicationController
  ...
  def create
    # Step 1: Check if the reservation exist (claim_id, host_id, host_id)
    ...
    @reservation = Reservation.where(
                    ...
                    claim_id: guest_review_params[:claim_id]
                   ).first

    if !@reservation.nil? && @reservation.claim.user.id == guest_review_params[:host_id].to_i
  ...
  private
    def guest_review_params
      params.require(:guest_review).permit(:comment, :star, :claim_id, :reservation_id, :host_id)
    end
end
